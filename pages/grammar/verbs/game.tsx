...
export default function VerbGame() {
  const [data, setData] = useState<VerbRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [round, setRound] = useState(1);
  const [question, setQuestion] = useState<VerbRow | null>(null);
  const [options, setOptions] = useState<VerbRow[]>([]);
  ...
  useEffect(() => {
    fetch('/data/Russian_Nouns.xlsx')
      ...
      .then(buffer => {
        ...
        if (!sheet) {
          console.error("❌ Sheet 'Verb' not found");
          return;
        }
        const rows: VerbRow[] = XLSX.utils.sheet_to_json(sheet, { defval: '', raw: true });
        setData(rows);
        setLoading(false);
      });
  }, []);
